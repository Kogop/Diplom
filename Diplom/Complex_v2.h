...
const double pi = 4.0 * atan(1.0);//3.1415926535897932384626433832795;
...
{
	double __e_im1 = exp(__z.im) / 2.0,
		__e_im2 = exp(-__z.im) / 2.0;
...
inline complex sqrt(const complex& __z)
{
	...
	cAr = cos(arg(__z) / 2.0);
	if (fabs(cAr) < pow(10.0, -15.0))
		cAr = 0.0;

	sAr = sin(arg(__z) / 2.0);
	if (fabs(sAr) < pow(10.0, -15.0))
		sAr = 0.0;
